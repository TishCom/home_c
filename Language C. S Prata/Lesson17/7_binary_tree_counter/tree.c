...
void DeleteAll(Tree *ptree)
{
    ...
    /*функции которые пользователь делает*/
/*--------------------------------------------*/

/*увеличивает значение счетчика элемента если он встретился снова*/
bool IncrementItem(const Item *pi, const Tree *ptree)
{
    Pair temp = SeekItem(pi, ptree);

    if (temp.child != NULL)
    {
        temp.child->item.counter++;
        return true;
    }
...
/*возвращает элемент в вызывающую функцию если он найден в дереве*/
bool GetItem(Item *pi, const Tree *ptree)
{
    Pair temp = SeekItem(pi, ptree);
