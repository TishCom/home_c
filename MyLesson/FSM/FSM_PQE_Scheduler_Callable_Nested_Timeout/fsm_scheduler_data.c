...
FSMSchedulerData* schedulerDataCreate(uint32_t priority, uint32_t quota)
{
    ...
    /* Инициализация работы с вызываемыми функциями */
...
/* ФУНКЦИИ ДЛЯ РАБОТЫ С ВЫЗЫВАЕМЫМИ ФУНКЦИЯМИ */

/* указание пранировщику ожидать окончания выполнения вызываемого автомата */
void schedulerDataSetWaiting(FSMSchedulerData *data)
{
    if (!schedulerDataIsInitialise(data))
        return;
...
/* указание пранировщику не ожидать окончания выполнения вызываемого автомата */
void schedulerDataResetWaiting(FSMSchedulerData *data)
{
    if (!schedulerDataIsInitialise(data))
        return;
...
/* проверка, должен ли планировщик ожидать окончания выполнения вызываемого */
bool schedulerDataIsWaiting(const FSMSchedulerData *data)
{
    if (!schedulerDataIsInitialise(data))
        return false;

    return data->is_waiting;
}
