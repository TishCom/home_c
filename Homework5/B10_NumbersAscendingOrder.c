...
{
	int32_t number = 0, oldDigits = 0, currentDigits = 0, flagAscending = 1;
	...
	oldDigits = RETURN_DIGIT_NUMBER(number, UNITS);
	
	while(number > 0)
	{
		number = SHIFT_DIGIT_NUMBER_RIGHT(number, UNITS);
		currentDigits = RETURN_DIGIT_NUMBER(number, UNITS);
