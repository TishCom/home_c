...
void inversionPartsArr(int arr[], int number, int size)
{
	for (int parts = 0, partFinal = 0; parts < number; parts++)
	{
		partFinal = parts == number - 1 ? size % number : 0;
		inversionArr(arr + parts * size / number, size / number + partFinal);
	}
}
